...
class ArticleInteractor: PresenterToInteractorArticleProtocol {
    ...
    func fetchArticleBySource(source: Source, query: String?, page: Int) {
        AF.request("\(NEWS_API_URL)/everything?sources=\(source.id)&apiKey=\(NEWS_API_KEY)&pageSize=10&page=\(page)&q=\(query ?? "")").responseDecodable(of: ArticleResponse.self) { (response) in
